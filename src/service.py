...
class NotificationService:
    ...
    def create_notification(self, target_channels, message):
        ...
        n.status = PENDING   # 👈 פה נכנס הסטטוס
    ...
    #def send_one(self, n):
        # כאן נשלח את ההודעה ונעדכן את הסטטוס בהתאם לתוצאה
     #   success = self._send_to_channels(n.targetChannels, n.message)
      #  if success:
       #     n.status = SENT
        #else:
         #   n.status = FAILED
    ...
    def send_one(self, nid):
        ...
        processor.send_one(n)
        return n
        #success = processor.send_one(n.target_channels, n.message)
        #if success:
         #   n.status = SENT
        #else:
         #   n.status = FAILED
